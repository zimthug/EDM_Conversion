 insert into callejero
  select usuario, f_actual, programa,
         (select max(cod_calle) + 1 from callejero) cod_calle,
         cod_prov, cod_depto, cod_munic, cod_local,
         'UNDEFINED MACHAVA PREPAGO' nom_calle, tip_via, cod_unicom,
         cod_oficom, ind_urb_rur, cod_post, cod_centec, tip_via2,
         nas_code, 'UNDEFINED MACHAVA PREPAGO' nom_calle_arab,
         ifs_dep_code
          from callejero
...
update int_supply
 ...
 insert into callejero
  select usuario, f_actual, programa,
         ...
         cod_oficom, ind_urb_rur, cod_post, cod_centec, tip_via2,
         nas_code, 'UNDEFINED MATOLA PREPAGO' nom_calle_arab,
         ifs_dep_code
          from callejero
...
update int_supply
 ...
 insert into callejero
  select usuario, f_actual, programa,
         ...
         cod_oficom, ind_urb_rur, cod_post, cod_centec, tip_via2,
         nas_code, 'UNDEFINED MAPUTO PREPAGO' nom_calle_arab,
         ifs_dep_code
          from callejero
