declare
  ...
  select to_char(tr.mes_ano, 'yyyymmdd') facture, tr.zona, tr.instalacao,
           ...
           nvl(lag(tr.mes_ano, 1)
                over(partition by sp.nis_rad order by sp.nis_rad, tr.mes_ano),
           ...
           sp.f_alta, tr.valor, tr.cobranca, 'TR050' tip_rec
 ...
 where exists (select *
   ...
   and tr.codoperacao in (630, 631, 632);
   ...
   /* select to_char(tr.mes_ano, 'yyyymmdd') facture, tr.zona, tr.instalacao,
           ...
           sp.f_alta, tr.valor, tr.cobranca, 'TR050' tip_rec
      from edmaccess.transitado tr, int_supply sp
     where \*tr.opedescricao = 'Feg'
           and *\
     tr.zona = sp.centre
     and tr.instalacao = sp.client
     and sp.nis_rad is not null
     and tr.valor != tr.cobranca
     and tr.valor > 0
     and (sp.nis_rad, tr.mes_ano) in
     (select nis_rad, mes_ano
        from edmaccess.transitado t, int_supply s
       where codoperacao = 632
         and exists (select 0
                from edmaccess.consumidores
               where zona = t.zona
                 and no_da_instalacao = t.instalacao
                 and activo = 1)
         and exists (select 0
                from edmaccess.contador
               where zona = t.zona
                 and no_da_instalacao = t.instalacao)
         and t.zona = centre
         and t.instalacao = client
         and t.valor != t.cobranca
      minus
      select nis_rad, f_fact
        from recibos);*/
  lf_proc_cobro       date;
...
begin
  ...
  for lcur_bill_rec in lcur_bill loop
    begin
      ...
      ll_excess      := 1;
      ls_tip_rec     := lcur_bill_rec.tip_rec;
      ...
      values
        (gls_usuario, trunc(sysdate), gls_programa, ll_sec_rec,
         ...
         lf_previous_date, 0, 'CU001', 'TR110', ll_imp_tot_rec,
